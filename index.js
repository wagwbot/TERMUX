...
const XPTN = 'RAMA'; // Jangan lu ubah² kalo gak mau error
const instagram = 'riska_gaming._'; //Jangan lu ubah² kalo gak mau error
const nomer = 'https://wa.me/+62895355566000'; // Jangan lu ubah² kalo gak mau error
const aktif = 'serah gw'; // Kapan bot lu aktif
    ...
    axios.get(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=${bg}`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[ WAIT ] Tunggu yha botlent sedang memperoses', MessageType.text, { quoted: m })
  ...
  var teks = text.replace(/%nulis2 /, '')
    axios.get(`https://mhankbarbars.herokuapp.com/nulis?text=${teks}&apiKey=N2Ws9kp3KTDYtry5Jjyz`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, 'Botmaslent Lagi Nulis ', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
...
conn.sendMessage(id, 'maslent Telah mengganti nama Grub' ,MessageType.text, { quoted: m } );
...
conn.sendMessage(id, 'maslent telah mrubah descripsi grup' ,MessageType.text, { quoted: m } );
...
conn.sendMessage(id, 'SUCCES, BOT telah membuka grub' ,MessageType.text, { quoted: m } );
}
else if (text == '.closegc'){
 let hasil = `${id.split("@s.whatsapp.net")[0]}`;
   conn.groupSettingChange (hasil, GroupSettingChange.messageSend, true);
conn.sendMessage(id, 'SUCCES, BOT TELAH MENUTUP GRUB' ,MessageType.text, { quoted: m } );
}
...
//jangan di ubah ubah sc BANGLENT
